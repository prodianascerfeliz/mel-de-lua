  ...
  return (
    <main style={{ backgroundColor: '#060d1a', minHeight: '100vh', fontFamily: "'Georgia', serif", color: '#FFFFFF' }}>
      ...
      <header style={{height:'64px',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 48px',backgroundColor:'rgba(6,13,26,0.9)',backdropFilter:'blur(16px)',borderBottom:'1px solid rgba(255,255,255,0.06)',position:'fixed',top:0,left:0,right:0,zIndex:100}}>
        <a href="/" style={{textDecoration:'none'}}><span style={{fontSize:'18px',fontWeight:300,letterSpacing:'0.35em',color:'#FFFFFF'}}>Mel de Lua</span></a>
        <a href="#cadastro" style={{backgroundColor:'#2E86C1',color:'#FFFFFF',padding:'9px 24px',fontSize:'11px',letterSpacing:'0.2em',textDecoration:'none',textTransform:'uppercase',fontFamily:'sans-serif'}}>Quero ser parceira</a>
      </header>
      <section className="hero-pad" style={{minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',padding:'120px 48px 80px',position:'relative',zIndex:10}}>
        <div className="fade-up" style={{maxWidth:'780px'}}>
          ...
          <div style={{display:'flex',gap:'20px',justifyContent:'center',flexWrap:'wrap'}}>
            <a href="#cadastro" style={{backgroundColor:'#2E86C1',color:'#FFFFFF',padding:'16px 48px',fontSize:'12px',letterSpacing:'0.2em',textDecoration:'none',textTransform:'uppercase',fontFamily:'sans-serif'}}>Quero ser parceira</a>
            <a href="#como-funciona" style={{fontSize:'13px',color:'rgba(255,255,255,0.45)',textDecoration:'none',letterSpacing:'0.08em',fontFamily:'sans-serif',display:'flex',alignItems:'center',gap:'8px'}}>Ver como funciona <span style={{color:'#F0A500'}}>↓</span></a>
          </div>
        </div>
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',backgroundColor:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.06)',marginTop:'80px',width:'100%',maxWidth:'640px'}}>
          {[{num:'0',label:'custo para entrar'},{num:'3',label:'agências por casal'},{num:'100%',label:'casais qualificados'}].map(item=>(
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
            {faqs.map((faq,i)=>(
              <div key={i} style={{border:'1px solid rgba(255,255,255,0.07)',overflow:'hidden'}}>
                <button className="faq-btn" onClick={()=>setFaqAberto(faqAberto===i?null:i)} style={{width:'100%',padding:'20px 24px',background:'none',border:'none',cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'16px',transition:'border-color 0.2s'}}>
                ...
                {faqAberto===i&&<div style={{padding:'0 24px 20px'}}><p style={{fontSize:'14px',color:'rgba(255,255,255,0.55)',fontFamily:'sans-serif',lineHeight:1.9,margin:0,fontWeight:300}}>{faq.a}</p></div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="cadastro" className="section-pad" style={{padding:'80px 64px',backgroundColor:'rgba(10,22,40,0.8)',position:'relative',zIndex:10}}>
        <div style={{maxWidth:'640px',margin:'0 auto'}}>
          <div style={{marginBottom:'40px',textAlign:'center'}}>
            ...
            <p style={{fontSize:'15px',color:'rgba(255,255,255,0.45)',fontFamily:'sans-serif',lineHeight:1.8,margin:0,fontWeight:300}}>Preencha o formulário e nossa equipe entrará em contato em até 48h para apresentar o modelo e iniciar o credenciamento.</p>
          </div>
          {!enviado?(
            <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:'16px'}}>
              <div className="form-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
                <div>
                  ...
                  <input type="text" value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} required placeholder="Nome completo" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
                <div>
                  ...
                  <input type="text" value={form.agencia} onChange={e=>setForm({...form,agencia:e.target.value})} required placeholder="Nome fantasia" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
              </div>
              <div className="form-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
                <div>
                  ...
                  <input type="text" value={form.cnpj} onChange={e=>setForm({...form,cnpj:e.target.value})} placeholder="00.000.000/0001-00" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
                <div>
                  ...
                  <input type="text" value={form.cidade} onChange={e=>setForm({...form,cidade:e.target.value})} required placeholder="Cidade / Estado" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
              </div>
              <div className="form-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px'}}>
                <div>
                  ...
                  <input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} required placeholder="contato@agencia.com" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
                <div>
                  ...
                  <input type="tel" value={form.telefone} onChange={e=>setForm({...form,telefone:e.target.value})} required placeholder="(11) 99999-9999" style={{width:'100%',padding:'12px 14px',backgroundColor:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.1)',color:'#FFFFFF',fontSize:'14px',fontFamily:'sans-serif',boxSizing:'border-box',transition:'border-color 0.2s'}}/>
                </div>
              </div>
              ...
              <button type="submit" disabled={enviando} className="submit-btn" style={{backgroundColor:'#2E86C1',color:'#FFFFFF',padding:'15px',fontSize:'12px',letterSpacing:'0.2em',border:'none',textTransform:'uppercase',fontFamily:'sans-serif',cursor:enviando?'wait':'pointer',transition:'background-color 0.2s',marginTop:'8px',opacity:enviando?0.7:1}}>
                {enviando?'Enviando...':'Enviar cadastro'}
              </button>
              <p style={{fontSize:'11px',color:'rgba(255,255,255,0.2)',textAlign:'center',fontFamily:'sans-serif',marginTop:'4px'}}>Retornamos em até 48h · Sem compromisso</p>
            </form>
          ):(
      ...
      <footer style={{borderTop:'1px solid rgba(255,255,255,0.06)',padding:'28px 48px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'12px',position:'relative',zIndex:10}}>
        <a href="/" style={{textDecoration:'none'}}><span style={{fontSize:'16px',fontWeight:300,letterSpacing:'0.3em',color:'rgba(255,255,255,0.4)'}}>Mel de Lua</span></a>
